...
function getArcParams() {
  ...
  return { radius: 420, arcHalf: Math.PI * 1.667 };
}
...
function openDetail(proj) {
  ...
  let heroHTML;
  if (proj.hero) {
    heroHTML = `<div class="det-hero"><img src="${proj.hero}" class="det-hero-img" alt="${proj.name}" onerror="this.parentNode.innerHTML='<div class=det-hero-placeholder>${proj.heroEmoji}</div>'"/></div>`;
  } else {
    heroHTML = `<div class="det-hero"><div class="det-hero-placeholder">${proj.heroEmoji}</div></div>`;
  }

  // Galería
  let galHTML = '';
  if (proj.gallery && proj.gallery.length > 0) {
    galHTML = proj.gallery.map(src =>
      `<div class="det-gallery-item"><img src="${src}" alt="${proj.name}" onerror="this.parentNode.innerHTML='<div class=det-gallery-placeholder>🖼</div>'"/></div>`
    ).join('');
  } else {
    // Placeholders generativos si no hay imágenes
    galHTML = [0,1,2,3].map(v => {
      const cv = document.createElement('canvas');
      const rotP = [...proj.palette.slice(v), ...proj.palette.slice(0,v)];
      const styles = ['mesh','dots','grid','lines'];
      drawGen(cv, {...proj, palette:rotP, style:styles[v%4]}, 600, 450);
      const wrap = document.createElement('div');
      wrap.className = 'det-gallery-item';
      wrap.appendChild(cv);
      cv.style.cssText = 'width:100%;height:100%;display:block;';
      return wrap.outerHTML.replace('</div>','') + cv.outerHTML + '</div>';
    }).join('');
  }
  ...
  cnt.innerHTML = `
    ...
    <div class="det-gallery" id="det-gal"></div>
  ...
  // Inyectar galería generativa (no puede ser innerHTML por canvas)
  if (!proj.gallery || proj.gallery.length === 0) {
    ...
    [0,1,2,3].forEach(v => {
      const cv = document.createElement('canvas');
      const rotP = [...proj.palette.slice(v), ...proj.palette.slice(0,v)];
      drawGen(cv, {...proj, palette:rotP, style:['mesh','dots','grid','lines'][v%4]}, 600, 450);
      cv.style.cssText = 'width:100%;height:100%;display:block;';
      const wrap = document.createElement('div');
      wrap.className = 'det-gallery-item';
      wrap.appendChild(cv);
